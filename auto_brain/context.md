# Phoenix Desktop Project Context
...
## Project Status
- **Kernel**: Rust code moved to `/rust-kernel`. Compiled into a native Node.js addon.
- **Electron**: Configured as the main entry point (`main.js`). Manages window lifecycle and provides the "bridge" to Rust.
- **Frontend**: Vue 3 application in `/frontend`. Communicates with Rust via `window.phoenix.kernel`.
- **Environment**: Compatible with Rust 1.85.0. Problematic crates are version-pinned in `rust-kernel/Cargo.toml`.

## Key Components
- `/rust-kernel/src/lib.rs`: Native Rust logic exposed via NAPI-RS macros.
- `/main.js`: Electron Main process. Loads the Rust kernel and handles window management.
- `/preload.js`: Secure bridge between the Electron main process and the Vue UI.
...
## Multi-Agent Notes
- **DO NOT** use `invoke` (Tauri-specific). Use `window.phoenix.kernel.execute()` for logic.
...
- Always run `npm run kernel:build` when changing Rust code to refresh the native bindings.