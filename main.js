 ...
 * This is the "Rust Kernel" - no HTTP server, no ports, zero latency.
 */
let kernel;
try {
  ...
  kernel = new Kernel();
  console.log('Rust Kernel successfully linked to Electron process.');
} catch (err) {
  console.error('Failed to load Rust Kernel:', err);
}
...
/**
 * IPC Bridge: The "Link" between UI and Rust Kernel
...
ipcMain.handle('kernel:execute', async (event, input) => {
  if (!kernel) return 'Error: Rust Kernel not loaded.';
